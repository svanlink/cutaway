...
other SwiftData app that names nothing also opens. One such app on this Mac
rewrote the file to its own schema three times in a week; Cutaway then
found no ZPROJECT and created empty tables. Not the test harness (proved:
the unit suite ran against a restored store and left it intact, though the
test host does open the real store — now quarantined too).

Restored, in `Cutaway/billing.store` (a named file in Cutaway's own folder,
...
scratchpad. Real prefs had been polluted by demo launches (selected project
"Nyx Fashion Film", primer marked shown) — CUTAWAY_DATA_DIR now quarantines
prefs as well as the store; the two keys were removed.
...
- A failed store write is a red sentence in the panel and the Stats window,
  logged as `store-error`, never a swallowed `try?` (`StoreErrorReporter`).
- Appearance is decided once — `NSApp.appearance = .darkAqua` in
  AppDelegate; the seven per-view `preferredColorScheme` forces are gone
  and a guard test keeps them gone. Found on the way: an NSPopover takes
  its appearance from the menu bar it hangs off, so the panel went light
  until it was told to inherit the app's decision.
- Bundle says what it is: category, copyright, version — all in
  project.yml, because xcodegen rewrites Info.plist on every generate
  (that had silently undone the 1.2.0 stamp one commit after it landed).
- String Catalog with German, generated by `scripts/strings.py` from the
  sources; AppKit strings through `String(localized:)`. Verified by a
  by-hand run with `AppleLanguages (de)`: panel, Stats and Settings read
  German; the pass found ten strings that reached `Text()` as `String`
  (verbatim) and routed them through the catalog.
- First-run permissions primer: what Cutaway asks for (Accessibility —
  optional, Automation — not yet) and what it never does. Shown once,
  never in a scenario run, reachable from Settings, audited by the UI test.
- Shortcuts.app actions: Pause, Resume, Today's Time, Switch Project.
- Frontmost app from `NSWorkspace.didActivateApplicationNotification`
  instead of a call every tick.
- Crash and hang reports via MetricKit to
  `~/Library/Application Support/Cutaway/Diagnostics`, bounded to ten,
  copyable from Settings. Nothing leaves the Mac.
- `.github/workflows/release.yml`: tag `vX.Y.Z` → test, build, ad-hoc
  sign, GitHub release, cask bump (needs the `TAP_TOKEN` secret).

Gate on HEAD: 337 unit tests (2 skipped), smoke ×3 ALL PASS, 8 UI tests (1 skipped) — all green at 6b1b5f7; graph 1,879 nodes / 4,102 edges / 105 communities.

Owed: Icon Composer icon (GUI + design pass); notarization/Sparkle when
decision 1 (no Developer ID) changes; AXObserver for Resolve titles; the
`TAP_TOKEN` repository secret; real-run checks of Shortcuts.app listing
the actions and of the frontmost tracker following a Resolve ↔ Adobe
switch (the harness's probes are fakes, and TextEdit/Finder are not
anchors, so the log stayed at `engine-start`); a first-run look at the
primer by the human.
